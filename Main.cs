    ...
    public partial class Main : Form
    {
        ...
        */
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string filePath = @"C:\Users\lucil\OneDrive\Masaüstü\KeyAuth-CSHARP-Example-main\CRInjector.exe";
            ...
            {
                // Hata durumunda bir mesaj kutusu gösterin
                MessageBox.Show("Dosya çalýþtýrýlamadý: " + ex.Message);
            }
        ...
        {
            // Kaynak dosya yolu
            string sourceFilePath = @"C:\Users\lucil\OneDrive\Masaüstü\KeyAuth-CSHARP-Example-main\crackedbyethereum.jar";

            // Hedef klasör ve dosya yolu
            string targetDirectory = @"C:\crackedbyethereum";
            string targetFilePath = Path.Combine(targetDirectory, "crackedbyethereum.jar");

            try
            {
                // Hedef klasörü oluþtur (eðer yoksa)
                if (!Directory.Exists(targetDirectory))
                {
                    Directory.CreateDirectory(targetDirectory);
                }

                // Dosyayý hedef klasöre kopyala
                File.Copy(sourceFilePath, targetFilePath, true);

                MessageBox.Show("Kurulum Baþarýlý PixelQuest Ýyi Oyunlar Diler :) ");
            }
            catch (Exception ex)
            {
                // Hata durumunda bir mesaj kutusu gösterin
                MessageBox.Show("Dosya kopyalanamadý: " + ex.Message);
            }
